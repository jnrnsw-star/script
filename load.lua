...
loadstring(game:HttpGet("https://raw.githubusercontent.com/jnrnsw-star/Scripts/main/load.lua"))()
]]
_G.TSUM = _G.TSUM or {}

local BASE = "https://raw.githubusercontent.com/jnrnsw-star/Scripts/main/tsum/"
local BASE_ALT = "https://github.com/jnrnsw-star/Scripts/raw/refs/heads/main/tsum/"
...
for i, name in ipairs(MODULES) do
    show(("Модуль %d/%d: %s"):format(i, #MODULES, name))
    local src, used
    for _, base in ipairs({ BASE, BASE_ALT }) do
        local ok, body = pcall(httpGet, base .. name)
        if ok and type(body) == "string" and #body > 200 and not body:find("<!DOCTYPE", 1, true) then
            src, used = body, base .. name
            break
        end
    end
    if not src then
        show("Не скачался: " .. name .. "\n\nЗалей папку tsum/ на GitHub\nВключи HttpGet в executor", true)
        return
    end
